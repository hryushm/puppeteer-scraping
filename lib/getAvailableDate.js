...
async function scrapingAvairableDate() {
  ...
  const isNightSlot = slotNum => slotNum === 4;
...
async function getAvairableDate() {
  ...
  await clickXpath(page, '//*[@id="contents"]/form[1]/div/div/div/div[1]/div/div[1]/select[1]/option[3]'); // open air sports facility
  ...
  const availableDate = await page.evaluate(scrapingAvairableDate);
